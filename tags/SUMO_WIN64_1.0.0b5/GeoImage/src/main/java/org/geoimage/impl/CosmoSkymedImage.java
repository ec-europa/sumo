...
public class CosmoSkymedImage extends SarImageReader {
    ...
    private void extractQuickLook() {
    	if(group==null){
    		String overview = h5file.getParentFile().getAbsolutePath() + File.separator + "preview_.jpg";
    		if (new File(overview).exists()) {
    			return;
    		}
    	}else{
	        try {
	        	String overview = h5file.getParentFile().getAbsolutePath() + File.separator + "preview_"+group+".jpg";
