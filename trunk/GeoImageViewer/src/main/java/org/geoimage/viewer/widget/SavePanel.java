...
public class SavePanel extends javax.swing.JPanel {
    ...
    public SavePanel(ISave layer) {
        ...
        if(layer instanceof SimpleVectorLayer)
            if(((SimpleVectorLayer)layer).getParent() instanceof IImageLayer){
            	OptionMenu opt=comboSaveFormat.getItemAt(comboSaveFormat.getSelectedIndex());
            	if (opt==null)
            		opt=comboSaveFormat.getItemAt(0);
                jTextField1.setText(((IImageLayer)((SimpleVectorLayer)layer).getParent()).getImageReader().getFilesList()[0] + "." + opt.getOptionDesc());
            }    
    }
