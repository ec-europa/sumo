...
public class GeoCoding {
	...
	private static org.slf4j.Logger logger=LoggerFactory.getLogger(GeoCoding.class);
	
	
	
	GeoCoding(String metaFile){
		meta =new S1Metadata();
	 ...
	 * @param lon
	 */
	public void reverse(double lat,double lon){
		...
		//double[][] statevVecInterp=orbitInterpolation.getStatevVecInterp();
		...
		*/
		...
		if (meta.getCoordinateConversion()!=null&&!meta.getCoordinateConversion().isEmpty()){
	    	...
	    	double newD=0;
            // Using the slantToGroundRangeCoefficients polynomial - This polynomial seems to be slightly wrong!!!!!
			// nCoeffs = length(slantToGroundRangeCoefficientsInterp);
			// vExps = (0:nCoeffs-1);
			// tmpD = SRdist - slantToGroundRangeOriginInterp;
			// D = (slantToGroundRangeCoefficientsInterp')*(tmpD.^vExps)';  % This polynomial transforms from slant range (in metres) to ground range (in metres)       
			//            
	...
	public void forward(double l, double p){
		double lon=0;
		double lat=0;
		...
		*/
		...
		if(meta.getPixelTimeOrdering().equalsIgnoreCase("Increasing")){
		...
		if (coordConv!=null&&!coordConv.isEmpty()){
			...
			for(int iidx=0; iidx<2;iidx++){ //First iteration with a default pH, second with a more accurate pH
				...
				for(;rEarthChange > 0.1;){
				    ...
				    double Rz = (normPt0*normPt0 + sRdist*sRdist - rEarth*rEarth) / (2*normPt0);//Rz = (norm(p_t0)^2 + SRdist^2 - rEarth^2) / (2*norm(p_t0));
				    double Rx = Rz * tanPsi;
				...
				logger.debug("lat:"+lat+ "  lon:"+lon+"  ph:"+pH);
			}	
		}
	 ...
	 */
	public static double[] findZeroDoppler(double[][] statepVecInterp,double[] pXYZ,double[] timeStampInterp){
			int iOptFactor = 10;
	...
	public static void main(String args[]){
		...
		String metaF="C:/tmp/sumo_images/test_interpolation/S1A_IW_GRDH_1SDV_20141016T173306_20141016T173335_002858_0033AF_FA6D.SAFE/annotation/s1a-iw-grd-vv-20141016t173306-20141016t173335-002858-0033af-001.xml";
		...
		GeoCoding gc=new GeoCoding(metaF);
		//gc.reverse(lat, lon);
