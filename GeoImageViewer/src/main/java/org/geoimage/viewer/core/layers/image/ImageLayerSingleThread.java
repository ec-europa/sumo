...
public class ImageLayerSingleThread implements ILayer {
	...
	public ImageLayerSingleThread(GeoImageReader gir) {
		...
		levels = (int) (Math.sqrt(
				Math.max(gir.getWidth() / Constant.TILE_SIZE_DOUBLE, gir.getHeight() / Constant.TILE_SIZE_DOUBLE))) + 1;
		maxlevels = (int) (Math.sqrt(Math.max(gir.getWidth() / (Constant.TILE_SIZE_DOUBLE * 2),
				gir.getHeight() / (Constant.TILE_SIZE_DOUBLE * 2)))) + 1; // massimo
																			// livello
																			// di
																			// zoom
		curlevel = levels;
		...
		xpadding = (((1 << levels) << 8) - gir.getWidth()) / 2; // is equal
																// to(int)((Math.pow(2,levels+8)-
																// gir.getWidth())/2);
		ypadding = (((1 << levels) << 8) - gir.getHeight()) / 2; // (int)((Math.pow(2,levels+8)-
																	// gir.getHeight())/2);
		...
		if (tileSize < Constant.TILESIZEPIXELS)
			tileSize = Constant.TILESIZEPIXELS;
	...
	private void createMatrixTileOrder() {
		arrayReadTilesOrder = new int[maxnumberoftiles][maxnumberoftiles];// contain the weight for
																			// reading tile sequence,
																			// 0 most important
		...
		if (activeGir != null) {
			...
			if (zoom >= 1) {
				...
				for (int lll = maxlevels; (lll > curlevel - 1); lll--) {
					...
					final String initfile = new StringBuffer(File.separator).append((int) lll).append(File.separator)
							.append((activeGir instanceof TiledBufferedImage
									? ((TiledBufferedImage) activeGir).getDescription() + File.separator : ""))
							.toString();

					// AG loads the different tiles, starting from the center
					// (k=0)
				...
				final String initfile = new StringBuilder(File.separator).append("0").append(File.separator)
						.append((activeGir instanceof TiledBufferedImage
								? ((TiledBufferedImage) activeGir).getDescription() + "/" : ""))
						.toString();
				for (int j = 0; j < max; j++) {
					if (j + h0 < 0) {
						continue;
					...
					for (int i = 0; i < max; i++) {
						...
						// start reading tiles in center of the image and go
						// through the borders
						...
						float xmax = (float) (((i + w0 + 1) * Constant.OVERVIEW_SIZE_DOUBLE - xx)
								/ (1d * width * zoom));

						// check if the tile is in or out, if is not visible
						// then is not loaded
						if (ymin > 1 || ymax < 0) {
							continue;
						}
	...
	public BufferedImage createTile(GeoImageReader gir, File outFile, int level, int xTile, int yTile, int activeBand) {
		...
		try {
			...
			nat = gir.readAndDecimateTile(x, y, (int) (Constant.TILE_SIZE_IMG_LAYER * zoom),
					(int) (Constant.TILE_SIZE_IMG_LAYER * zoom), Constant.TILE_SIZE_IMG_LAYER,
					Constant.TILE_SIZE_IMG_LAYER, ((SarImageReader) gir).getWidth(), ((SarImageReader) gir).getHeight(),
					true, activeBand);
	...
	/*
	 * private String getBandFolder(int band) { StringBuilder out = new
	 * StringBuilder(); out.append(band); return out.toString(); }
	 */
	/**
	 *
	 * @param initfile
	 ...
	 * @param j
	 *
	 *            public void addTileToQueue(final String initfile, final int
	 *            level, final int i, final int j) { String tilesStr=new
	 *            StringBuffer(level).append(" "
	 *            ).append(getBandFolder(activeBand)).append(" "
	 *            ).append(i).append(" ").append(j).toString(); if
	 *            (!submitedTiles.contains(tilesStr)) {
	 *            submitedTiles.add(tilesStr);
	 * 
	 *            futures.add(0, poolExcutorService.submit(new
	 *            ServiceTile(initfile, level, i, j)));
	 * 
	 *            } }
	 */
	...
	public void setName(GeoImageReader gir) {
		if (gir.getDisplayName(activeBand) != null && !gir.getDisplayName(activeBand).equals(""))
			...
			if (gir instanceof AbstractCosmoSkymedImage
					&& ((AbstractCosmoSkymedImage) gir).getInternalImage() != null) {
