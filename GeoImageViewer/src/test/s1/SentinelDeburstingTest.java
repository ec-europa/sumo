...
public class SentinelDeburstingTest {
    private static Logger logger=LoggerFactory.getLogger(SentinelDeburstingTest.class);
	...
	SentinelDeburstingTest(String pathSafe,String pathXml,String pathTiff){
		init(pathSafe, pathXml);
		this.pathTiff=pathTiff;
	}
	
	public void init(String file,String annotationFile){
		try {
			safeReader=new SumoJaxbSafeReader(file);
			safeReader.getSwaths();
			
			annotation=new SumoAnnotationReader(annotationFile);
	...
	public void readBurstInformation(){
		...
		System.out.println("Burst elements informations: "+support.length);
	}
	
	public void processBurstInformation(){

		for(int i=0;i<support.length;i++){
			String[] fvSamples=support[i].firstValidSamples.split(" ");
	 ...
	 * @param imagePath
	 */
    public void saveInNewTiff() {
    	// Get Writer for TIFF
    	Iterator writers = ImageIO.getImageWritersByFormatName("tiff");
    	ImageWriter writer = (ImageWriter)writers.next();
    	
    	// Set WriteParam's
    	it.geosolutions.imageio.plugins.tiff.TIFFImageWriteParam writeParam = (it.geosolutions.imageio.plugins.tiff.TIFFImageWriteParam)writer.getDefaultWriteParam();
    	...
    	try{
        	...
        	// Create the new stream metadata object for big endian byte order.
        	...
        	byteOrderNode.setAttribute("value", "BIG_ENDIAN");
        	...
        	reader.setInput(new ByteArrayInputStream(support[0].databurst));
        	
        	BufferedImage newImage=new BufferedImage(reader.getWidth(0),reader.getHeight(0),reader.getImageTypes(0).next().getBufferedImageType());
        	        	
        	// Create IIOImage
        	IIOImage iioImage = new IIOImage( newImage.getData() 	,null   	,null);
	...
	public static void main(String args[]){
		...
		SentinelDeburstingTest s1=new SentinelDeburstingTest(pathSafe,pathXml,pathTiff);
