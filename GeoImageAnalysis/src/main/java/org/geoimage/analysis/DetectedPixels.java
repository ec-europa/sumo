...
public class DetectedPixels {
	...
	// private Map<String, BoatPixel> allDetectedPixels = new HashMap<String,
	// BoatPixel>();
	...
	public void add(int x, int y, int value, double tileAvg, double tileStd, double threshold, int band) {
		...
		// StringBuilder point=new StringBuilder("").append(x).append("
		// ").append(y);
	...
	/*
	 * public Boat[] getBoats() { return boatArray; }
	 */
	 ...
	 * @return
	 */
	public boolean checkNeighbours(List<int[]> pixels, int[][] imagedata, double[][] thresholdaggregate, int[] position,
			int neighboursdistance, int tilesize, int[] rastermask) {
			...
			for (int i = localpixel[0] - neighboursdistance; i < localpixel[0] + neighboursdistance + 1; i++) {
				for (int j = localpixel[1] - neighboursdistance; j < localpixel[1] + neighboursdistance + 1; j++) {
					...
					if (imagemap[i + j * tilesize] == 0) {
						...
						if ((rastermask == null) || (rastermask[i + j*tilesize] == 0)) {
							boolean aggregated = false;
							boolean clipped = false;
							int value = 0;

							// calculate the max pixel value for all bands -->
	...
	public void computeBoatsAttributes(String polarization) {
		...
		for (List<int[]> agBoat : enumeration) {
			...
			if (it.length > 0) {
				pixel = it[0];
				// store in the table the boat values and the estimated size,
				// heading and bearing of the aggregate
				String id = new StringBuilder().append(pixel[0]).append(" ").append(pixel[1]).toString();
				BoatPixel pixelValue = aggregatedPixels.get(id);

				// boatvalues[0]=number of pixel boatvalues[1-2]= posx e posy
				// boatvalues[3-4-5]=length,width,heading
				Boat boatValue = new Boat(pixelValue.id, boatvalues[1], boatvalues[2], boatvalues[0], boatvalues[3],
						boatvalues[4], boatvalues[5], pixelValue.value, pixelValue.tileAvg, pixelValue.tileStd,
				...
				// check if agglomerated boat has a width or a length larger
				// than filterSize
				if (boatValue.getLength() > ConstantVDSAnalysis.filterminSize
						&& boatValue.getWidth() > ConstantVDSAnalysis.filterminSize) { // AG
																						// changed
																						// ||
																						// to
																						// &&

					// check if agglomerated boat has a width or a length lower
					// than filterSize
					if (boatValue.getLength() < ConstantVDSAnalysis.filtermaxSize
							&& boatValue.getWidth() < ConstantVDSAnalysis.filtermaxSize) {
